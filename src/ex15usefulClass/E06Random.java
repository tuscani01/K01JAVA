...
public class E06Random
{
	...
	{
		/*
		 컴퓨터는 인자로 전달되는 Seed(씨앗)을 기반으로 난수 생성한다 만약 Seed값이 동일하다면 항상 동일한 패턴의 난수만 생성되는데
	...
	for (int i = 0; i < 10; i++) {
		/*
		 nextInt()의 인자값 100이 전달되면 0~99사이 난수가 생성된다
		 */
	...
	System.out.println("###난수생성3 : 항상다른seed를 사용");
	Random random3 = new Random();
	//현재시간 밀리초 단위로 반환하는 메소드
	...
	"Random random4 = new Random();
			...
			for (int i = 0; i < 10; i++) {
				/*
				 nextInt()에 인자값 없는경우 음수,양수가 혼용되어 난수가 생성된다
				 */
